...
```{r}
train_index <- sample(n, size = floor(2/3 * n), replace=F)
train <- toyDF[train_index, ]
test <- toyDF[-train_index, ]
```
...
```{r}
trainPredictionRSME <- RMSE(as.matrix(trainPredictions), train_raw_avg)
trainPredictionRSME
testPredictionRSME <- RMSE(as.matrix(testPredictions), train_raw_avg)
testPredictionRSME
```
